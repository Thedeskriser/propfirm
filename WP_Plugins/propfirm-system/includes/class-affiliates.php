...
class FXSIM_Affiliates {
    ...
    public static function record_commission(int $referred_user_id, int $order_id, float $base_amount): void {
        ...
        // Dynamic Gamified Tiers (Level 1: 10%, Level 2: 15%, Level 3: 20%)
        $past_sales = (int) $wpdb->get_var($wpdb->prepare(
            "SELECT COUNT(*) FROM {$wpdb->prefix}fxsim_commissions WHERE affiliate_id = %d", $affiliate_id));
        
        $rate = self::default_rate(); // Fallback/base rate
        if ($past_sales >= 50) {
            $rate = 20.0;
        } elseif ($past_sales >= 10) {
            $rate = 15.0;
        } else {
            $rate = 10.0;
        }
