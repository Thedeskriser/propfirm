...
class FXSIM_Coupons {
    ...
    public static function validate(string $code, int $plan_id, int $user_id): array {
        ...
        if (isset($c->min_cart_value) && (float) $c->min_cart_value > 0 && $price < (float) $c->min_cart_value) {
            return ['valid' => false, 'message' => 'Minimum order amount not met for this coupon.'];
        }

        if (isset($c->first_time_only) && (int) $c->first_time_only === 1) {
            $past_orders = (int) $wpdb->get_var($wpdb->prepare(
                "SELECT COUNT(*) FROM {$wpdb->prefix}fxsim_payment_orders WHERE user_id = %d AND status = 'approved'",
                $user_id));
            if ($past_orders > 0) {
