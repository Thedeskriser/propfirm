...
class FXSIM_Emails {
    ...
    private static function build(string $event, array $d, array $ctx): array {
        ...
        return match($event) {
            ...
            'bounce_back_offer' => [
                'Exclusive 20% OFF Bounce Back Offer — ' . $brand,
                "<h2 style='font-size:22px;font-weight:800;color:#111827;margin:0 0 8px'>Get Back in the Game</h2>"
                . "<p style='color:#6b7280;margin:0 0 24px'>Hi {$name}, we noticed your recent challenge ended. Don't let a small setback stop your trading journey.</p>"
                . "<p>The best traders learn from their losses. To help you get back on track, here's an exclusive <strong>20% discount</strong> on your next challenge.</p>"
                . "<div style='background:#f9fafb;border:1px dashed #7c6ef5;border-radius:10px;padding:20px;text-align:center;margin:24px 0'>"
                . "<p style='color:#9ca3af;font-size:13px;text-transform:uppercase;letter-spacing:1px;margin:0 0 8px;font-weight:700'>Use Code at Checkout</p>"
                . "<p style='color:#111827;font-size:24px;font-weight:800;letter-spacing:2px;margin:0 font-family:monospace'>BOUNCE20</p>"
                . "</div>"
                . "<p style='color:#6b7280;font-size:14px;margin:0 0 24px'>Code is valid for the next 48 hours only.</p>"
                . "<div style='text-align:center;margin:28px 0'>" . self::btn('Start New Challenge', $chall . '?coupon=BOUNCE20') . "</div>",
            ],
            default => ['', ''],
        };
    }
...
<body style='margin:0;padding:0;background:#f3f4f6;font-family:-apple-system,BlinkMacSystemFont,\"Segoe UI\",Helvetica,Arial,sans-serif'>
  ...
  <table cellpadding='0' cellspacing='0' width='100%' style='background:#f3f4f6'>
    ...
    public static function send_daily_owner_report(): bool {
        global $wpdb;
        $admin = get_option('fxsim_admin_email', '') ?: get_option('admin_email');
        if (!$admin) return false;
        ...
        // 24-hour stats
        $rev = (float) $wpdb->get_var("SELECT SUM(amount) FROM {$wpdb->prefix}fxsim_payments WHERE status='approved' AND updated_at >= NOW() - INTERVAL 1 DAY");
        $payouts = (float) $wpdb->get_var("SELECT SUM(amount) FROM {$wpdb->prefix}fxsim_payouts WHERE status='paid' AND updated_at >= NOW() - INTERVAL 1 DAY");
        $net = $rev - $payouts;
        
        $new_accts = (int) $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->prefix}fxsim_challenge_accounts WHERE phase=1 AND phase_started_at >= NOW() - INTERVAL 1 DAY");
        $blown = (int) $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->prefix}fxsim_challenge_accounts WHERE status='failed' AND updated_at >= NOW() - INTERVAL 1 DAY");
        $funded = (int) $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->prefix}fxsim_challenge_accounts WHERE status='funded' AND funded_at >= NOW() - INTERVAL 1 DAY");

        $fRev = '$' . number_format($rev, 2);
        $fPay = '$' . number_format($payouts, 2);
        $fNet = '$' . number_format($net, 2);
        $netColor = $net >= 0 ? '#16a34a' : '#dc2626';

        $body = "
        <div style='text-align:center;margin-bottom:24px'>
            <p style='font-size:12px;color:{$color};font-weight:700;letter-spacing:1.5px;text-transform:uppercase;margin:0 0 8px'>Daily Report</p>
            <h2 style='font-size:24px;font-weight:800;color:#111827;margin:0'>End of Day Summary</h2>
            <p style='color:#6b7280;font-size:14px;margin-top:4px'>" . date('F j, Y') . "</p>
        </div>
        
        <div style='background:#f9fafb;border:1px solid #e5e7eb;border-radius:8px;padding:20px;margin-bottom:20px'>
            <table width='100%' cellpadding='0' cellspacing='0'>
                <tr>
                    <td style='padding-bottom:12px'><span style='color:#6b7280;font-size:14px'>Total Revenue (24h)</span></td>
                    <td align='right' style='padding-bottom:12px'><strong style='font-size:16px;color:#111827'>{$fRev}</strong></td>
                </tr>
                <tr>
                    <td style='padding-bottom:12px;border-bottom:1px solid #e5e7eb'><span style='color:#6b7280;font-size:14px'>Payouts Sent (24h)</span></td>
                    <td align='right' style='padding-bottom:12px;border-bottom:1px solid #e5e7eb'><strong style='font-size:16px;color:#dc2626'>-{$fPay}</strong></td>
                </tr>
                <tr>
                    <td style='padding-top:12px'><strong style='color:#111827;font-size:15px'>Net Profit</strong></td>
                    <td align='right' style='padding-top:12px'><strong style='font-size:18px;color:{$netColor}'>{$fNet}</strong></td>
                </tr>
            </table>
        </div>
        ...
        <div style='background:#f9fafb;border:1px solid #e5e7eb;border-radius:8px;padding:20px;margin-bottom:24px'>
            <table width='100%' cellpadding='0' cellspacing='0'>
                <tr>
                    <td style='padding-bottom:12px'><span style='color:#6b7280;font-size:14px'>New Challenges</span></td>
                    <td align='right' style='padding-bottom:12px'><strong style='font-size:15px;color:#111827'>{$new_accts}</strong></td>
                </tr>
                <tr>
                    <td style='padding-bottom:12px'><span style='color:#6b7280;font-size:14px'>Newly Funded</span></td>
                    <td align='right' style='padding-bottom:12px'><strong style='font-size:15px;color:#16a34a'>{$funded}</strong></td>
                </tr>
                <tr>
                    <td><span style='color:#6b7280;font-size:14px'>Blown Accounts</span></td>
                    <td align='right'><strong style='font-size:15px;color:#dc2626'>{$blown}</strong></td>
                </tr>
            </table>
        </div>
        
        <div style='text-align:center'>" . self::btn('Open Dashboard', self::frontend_base() . '/dashboard/admin') . "</div>";

        $html = self::wrap($body, $ctx);
        return wp_mail($admin, "[{$brand}] Daily Owner Report", $html, ['Content-Type: text/html; charset=UTF-8']);
    }
}
