...
class FXSIM_Trading_Engine {
    ...
    public static function open_position(int $user_id, array $args): array {
        ...
        // ── Plan rules: single JOIN fetches news_trading + lot/leverage caps + new rules ────────
        ...
        $plan_rules = $wpdb->get_row($wpdb->prepare(
            "SELECT cp.news_trading, cp.weekend_holding, cp.ea_allowed, cp.hedging_allowed, cp.copy_trading_allowed, cp.stop_loss_required, cp.max_lot_size, cp.max_leverage, cp.martingale_allowed, cp.news_window_minutes
        ...
        if (get_option('fxsim_news_lock', false) && $plan_rules && !(int)$plan_rules->news_trading) {
            $news_mins = (int)($plan_rules->news_window_minutes ?? 5);
            return self::err("\u26a0 News lock active. Trading paused {$news_mins} minutes before/after high-impact news events.");
        }
        ...
        if ($plan_rules && isset($plan_rules->stop_loss_required) && (int)$plan_rules->stop_loss_required) {
            if ($sl === null) {
                return self::err('Stop Loss is mandatory for this challenge plan. Please specify a Stop Loss.');
            }
        }

        // Hedging Allowed Rule
        if ($plan_rules && isset($plan_rules->hedging_allowed) && !(int)$plan_rules->hedging_allowed) {
            $opposite = $type === 'buy' ? 'sell' : 'buy';
            $has_hedge = $wpdb->get_var($wpdb->prepare(
                "SELECT id FROM {$wpdb->prefix}fxsim_positions WHERE account_id=%d AND symbol=%s AND type=%s LIMIT 1",
                $account->id, $args['symbol'], $opposite
            ));
            if ($has_hedge) {
                return self::err("Hedging is not allowed on this plan. You already have a {$opposite} position open for {$args['symbol']}.");
            }
        }
    ...
    public static function close_position(int $user_id, int $pos_id, string $reason = 'manual'): array {
        ...
        // HFT Protection: Configurable minimum holding time for manual closures
        $min_trade_secs = 15; // default
        if (class_exists('FXSIM_Challenge_DB')) {
            $ch = $wpdb->get_row($wpdb->prepare("SELECT plan_id FROM {$wpdb->prefix}fxsim_challenge_accounts WHERE fxsim_account_id=%d AND status IN ('active','funded')", $account->id));
            if ($ch) {
                $plan = FXSIM_Challenge_DB::get_plan((int)$ch->plan_id);
                if ($plan && isset($plan->min_trade_seconds)) {
                    $min_trade_secs = (int)$plan->min_trade_seconds;
                }
            }
        }
        
        $hold_time = time() - strtotime($pos->opened_at . ' UTC');
        if ($reason === 'manual' && $hold_time < $min_trade_secs) {
            return self::err("Trade must be held for at least {$min_trade_secs} seconds to prevent HFT/latency abuse.");
        }
    ...
    public static function check_sl_tp(): void {
        ...
        // Lock expires after 60s even if execution crashes (prevents permanent lock)
