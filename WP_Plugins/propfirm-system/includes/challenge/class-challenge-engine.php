...
class FXSIM_Challenge_Engine {
    ...
    public static function evaluate_after_trade(int $challenge_id): void {
        ...
        // ── Check 0: Margin Call (Stop Out) ────────────────────────────────────
        $margin_used = (float)$account->margin_used;
        $margin_call_level = isset($plan->margin_call_level) ? (float)$plan->margin_call_level : 50.0;
        
        if ($margin_used > 0) {
            $margin_level = ($current_equity / $margin_used) * 100;
            if ($margin_level < $margin_call_level) {
                // Close all open positions immediately (handled internally by breach)
                self::breach($challenge_id, (int)$challenge->user_id, 'margin_call',
                    $margin_call_level, round($margin_level, 2),
                    "Margin Call Stop Out: Margin level dropped to " . round($margin_level, 2) . "% (below {$margin_call_level}% limit). All open positions have been liquidated.");
                return;
            }
        }
    ...
    public static function daily_tasks(): void {
        ...
        foreach ($challenges as $ch) {
            ...
            if (isset($plan->max_inactivity_days) && (int)$plan->max_inactivity_days > 0) {
                $last_trade = $ch->last_trade_date ? strtotime($ch->last_trade_date) : strtotime($ch->phase_started_at);
                $days_inactive = (time() - $last_trade) / 86400;
                if ($days_inactive > (int)$plan->max_inactivity_days) {
                    self::breach((int)$ch->id, (int)$ch->user_id, 'inactivity',
                        (int)$plan->max_inactivity_days, round($days_inactive),
                        "Inactivity limit reached: no trades for " . round($days_inactive) . " days (limit: {$plan->max_inactivity_days}).");
                    continue;
                }
            }
    ...
    private static function breach(int $challenge_id, int $user_id, string $rule, $limit, $actual, string $description): void {
        ...
        // Close all open positions before locking the account
        ...
        if (class_exists('FXSIM_Emails')) {
            FXSIM_Emails::send($user_id, 'bounce_back_offer');
        }
    }
