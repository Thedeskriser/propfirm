...
class FXSIM_REST_API {
    ...
    public static function routes(): void {
        ...
        register_rest_route(self::NS, '/admin/payouts/execute-bulk', ['methods'=>'POST','callback'=>[self::class,'admin_payouts_execute_bulk'], 'permission_callback'=>$is_admin]);
        ...
        register_rest_route(self::NS, '/admin/risk/alerts',                         ['methods'=>'GET', 'callback'=>[self::class,'admin_risk_alerts'],       'permission_callback'=>$is_admin]);
                // Helpdesk & CRM Ticket System
                // Theme Settings (No-Code Editor)
 ...
 public static function admin_check(): bool {
   ...
   private static function verify_nonce(WP_REST_Request $r): bool {
    ...
    /** POST /admin/payouts/execute-bulk — Fetches all 'approved' payouts and simulates 
     *  a bulk crypto payment (e.g. Binance/Confirmo), then marks them 'paid'. */
    public static function admin_payouts_execute_bulk(WP_REST_Request $r): WP_REST_Response {
        global $wpdb;
        $table = $wpdb->prefix . 'fxsim_payouts';
        
        $approved = $wpdb->get_results("SELECT id FROM $table WHERE status = 'approved'");
        if (!$approved) {
            return new WP_REST_Response(['success' => false, 'message' => 'No approved payouts found.'], 400);
        }
        
        $ok = 0; $fail = 0;
        foreach ($approved as $row) {
            // Mock crypto API call
            $mock_tx_hash = '0x' . substr(md5(uniqid('', true)), 0, 32);
            
            $sub = new WP_REST_Request('POST', '');
            $sub->set_param('id', $row->id);
            $sub->set_body_params([
                'status' => 'paid', 
                'note' => 'Paid via 1-Click Crypto Integration.',
                'tx_reference' => $mock_tx_hash
            ]);
            $res = self::admin_payout_status($sub);
            $data = $res->get_data();
            if (!empty($data['success'])) $ok++; else $fail++;
        }
        FXSIM_Database::log_admin(get_current_user_id(), 'bulk_crypto_payout', 0, "$ok paid, $fail failed");
        return new WP_REST_Response(['success' => true, 'processed' => $ok, 'failed' => $fail]);
    }
    ...
    public static function admin_plan_save(WP_REST_Request $r): WP_REST_Response {
        ...
        $allowed = ['name','account_size','price','phases',
                    ...
                    'ea_allowed','hedging_allowed','copy_trading_allowed','stop_loss_required',
                    'max_inactivity_days','ip_matching_required','martingale_allowed',
                    'refundable_fee','news_window_minutes','evaluation_profit_share',
                    'min_trade_seconds','margin_call_level',
    ...
    // ── Risk & Fraud Alerts (Admin) ──────────────────────────────────────────
    public static function admin_risk_alerts(WP_REST_Request $r): WP_REST_Response {
        global $wpdb;
        
        // 1. High Frequency Trading behavior (Trades closed in under 30 seconds)
        // Find users with more than 5 such trades
        $hft = $wpdb->get_results("
            SELECT a.user_id, COUNT(t.id) as hft_trades
            FROM {$wpdb->prefix}fxsim_trade_history t
            JOIN {$wpdb->prefix}fxsim_accounts a ON t.account_id = a.id
            WHERE TIMESTAMPDIFF(SECOND, t.opened_at, t.closed_at) < 30
            GROUP BY a.user_id
            HAVING hft_trades > 5
            ORDER BY hft_trades DESC
            LIMIT 10
        ");

        // 2. Open Trade Flags
        $flags = $wpdb->get_var("SELECT COUNT(*) FROM {$wpdb->prefix}fxsim_trade_flags WHERE resolved = 0");
        
        // 3. High Lot Sizes (Gambling Risk) - trades with lot size > 20
        $gambling = $wpdb->get_results("
            SELECT a.user_id, COUNT(t.id) as massive_trades
            FROM {$wpdb->prefix}fxsim_trade_history t
            JOIN {$wpdb->prefix}fxsim_accounts a ON t.account_id = a.id
            WHERE t.lot_size > 20
            GROUP BY a.user_id
            HAVING massive_trades > 0
            ORDER BY massive_trades DESC
            LIMIT 10
        ");

        // Enhance with user data
        $map_users = function($rows, $key) {
            foreach ($rows as $row) {
                $user = get_userdata((int)$row->user_id);
                $row->user_email = $user ? $user->user_email : 'Unknown';
                $row->count = (int)$row->$key;
            }
            return $rows;
        };
