  ...
  return (
    <>
              ...
              {challenges.map((c, i) => {
                ...
                return (
                  <motion.tr
                    ...
                    className={cn("border-b border-border-subtle/40 last:border-0 hover:bg-surface-muted/30 transition-colors", isSelected && "bg-accent/10")}
                  >
                    <Td>
                    ...
                    <Td><Badge tone={statusTone(c.status)} pulsing={c.status === 'active' || c.status === 'funded'}>{statusLabel(c.status)}</Badge></Td>
      ...
      <FloatingActionBar
        ...
        actions={
          <>
            <Button size="sm" variant="outline" className="h-8">Suspend</Button>
            <Button size="sm" variant="danger" className="h-8">Reset</Button>
            <Button size="sm" variant="success" className="h-8">Upgrade</Button>
          </>
        }
      />
    </>
...
function Th({ children, align, hideOn, className }: { children: React.ReactNode; align?: 'right' | 'center'; hideOn?: 'sm' | 'md' | 'lg', className?: string }) {
  return (
    <th className={cn(
