...
import type { AdminStats, PaymentOrder, ChallengeAccount, AdminRiskAlerts } from '@/types/api'
...
function RiskAlerts() {
  const [alerts, setAlerts] = useState<AdminRiskAlerts | null>(null)
...
function GlobalExposureHeatmap() {
  const [exposure, setExposure] = useState<import('@/types/api').RiskExposureItem[] | null>(null)
  ...
  const maxLots = Math.max(...exposure.map(e => toNum(e.total_lots)), 1)
