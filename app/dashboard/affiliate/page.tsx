  ...
  return (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-text-muted mt-1">Your current commission rate is <span className="text-text font-bold text-accent">{me.rate_percent}%</span> of each referred purchase.</p>
      </div>

      {/* Gamified Tier Progress */}
      <Card className="bg-gradient-to-r from-bg-subtle to-surface border-accent/20">
        <CardContent className="p-6">
          <div className="flex justify-between items-end mb-2">
            <div>
              <div className="text-xs uppercase tracking-wider text-text-muted font-bold">Current Tier</div>
              <div className="text-xl font-bold text-accent">
                {s.conversions >= 50 ? 'Level 3 (20%)' : s.conversions >= 10 ? 'Level 2 (15%)' : 'Level 1 (10%)'}
              </div>
            </div>
            <div className="text-right">
              <div className="text-xs text-text-muted">Next Tier</div>
              <div className="text-sm font-medium">
                {s.conversions >= 50 ? 'MAX Level Reached' : s.conversions >= 10 ? 'Level 3 (20%)' : 'Level 2 (15%)'}
              </div>
            </div>
          </div>
          ...
          <div className="h-3 w-full bg-surface-muted rounded-full overflow-hidden border border-border-subtle">
            <div 
              ...
              style={{ width: `${s.conversions >= 50 ? 100 : s.conversions >= 10 ? ((s.conversions - 10) / 40) * 100 : (s.conversions / 10) * 100}%` }}
            />
          </div>
          
          <div className="mt-3 text-xs text-text-muted text-center">
            {s.conversions >= 50 ? (
              'You have reached the maximum commission tier! Incredible work.'
            ) : s.conversions >= 10 ? (
              <><strong>{50 - s.conversions}</strong> more sales to unlock Level 3 (20% commission)!</>
            ) : (
              <><strong>{10 - s.conversions}</strong> more sales to unlock Level 2 (15% commission)!</>
            )}
          </div>
        </CardContent>
      </Card>
      ...
      <Card>
        <CardHeader>
          ...
          <p className="text-xs text-text-muted">Use these official banners and graphics on your social media, blog, or videos to boost your conversions.</p>
        </CardHeader>
        <CardContent>
          <div className="grid sm:grid-cols-2 gap-4">
            <div className="rounded-lg border border-border-subtle p-3 flex flex-col gap-3">
              <div className="aspect-video bg-surface-muted rounded border border-border flex items-center justify-center relative overflow-hidden group">
                {/* Placeholder for actual marketing graphic */}
                <div className="absolute inset-0 bg-gradient-to-br from-accent/20 to-bg flex items-center justify-center">
                  <span className="font-bold text-xl opacity-50 tracking-widest">PROP FIRM</span>
                </div>
              ...
              <div className="text-xs font-medium">16:9 Youtube / Twitter Banner</div>
            </div>
            ...
            <div className="rounded-lg border border-border-subtle p-3 flex flex-col gap-3">
              <div className="aspect-square bg-surface-muted rounded border border-border flex items-center justify-center relative overflow-hidden group">
                 ...
                 <div className="absolute inset-0 bg-gradient-to-tr from-info/20 to-bg flex items-center justify-center">
                  <span className="font-bold text-lg opacity-50 tracking-widest">IG POST</span>
                </div>
                <div className="absolute inset-0 bg-bg/80 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                  <Button variant="outline" size="sm">Download PNG</Button>
                </div>
              </div>
              <div className="text-xs font-medium">1:1 Instagram / Feed Post</div>
            </div>
          </div>
        </CardContent>
      </Card>
