...
export function PayoutReviewQueue() {
  ...
  const runCryptoBulk = async () => {
    if (!confirm('Are you sure you want to instantly pay all approved payouts via crypto?')) return
    setBulkBusy(true)
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-2 max-w-xs w-full">
          ...
          {filter === 'approved' && rows && rows.length > 0 && (
            <Button onClick={runCryptoBulk} loading={bulkBusy} variant="accent" className="shrink-0">
