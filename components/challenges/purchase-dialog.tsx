...
export function PurchaseDialog({ plan, open, onClose }: Props) {
  ...
  useEffect(() => {
    if (!open) {
      ...
      if (urlCoupon && !couponInfo) {
        setCoupon(urlCoupon);
        // We can't directly call applyCoupon here because it uses state that might not be updated, 
        // but we can call the API directly.
        setCouponBusy(true); setCouponErr(null);
        api.couponValidate(urlCoupon.trim(), plan.id).then(res => {
          setCouponBusy(false);
          if (res.ok && res.data.valid) {
            setCouponInfo({ code: res.data.code || urlCoupon.trim().toUpperCase(), original: res.data.original ?? price, discount: res.data.discount ?? 0, final: res.data.final ?? price })
          } else {
            setCouponInfo(null)
            setCouponErr(res.ok ? res.data.message : (res.error || 'Could not validate coupon'))
          }
